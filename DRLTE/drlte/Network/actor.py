...
from tensorflow.compat.v1.logging import log,DEBUG,INFO
...
class ActorNetwork:
    def __init__(self, sess, dim_state, dim_action, bound_action, learning_rate, tau, num_path):
        ...
        #print("-----------------------------------------------------------")
        #print("__paras", self.__paras.get_shape().as_list())
        ...
        #print("__target_paras", '', self.__paras.get_shape().as_list())
        #print("-----------------------------------------------------------")
    ...
    def buildNetwork(self):
        ...
        log(DEBUG,"******Build full connected network（Actor）!******")
    ...
    def buildNetwork(self):
        ...
        # for more or less parameters
        #net = tf.contrib.layers.fully_connected(net, 128, activation_fn=tf.nn.leaky_relu)
        print("*******This is actor network building(cnn_depth_s_dim)!*******")      
        ...
        out_vec = []
        for i in self.__num_path:
            #out = tf.layers.dense(inputs=net, units=i, activation=tf.nn.tf.nn.softmax) 
            out = tf.contrib.layers.fully_connected(net, i, activation_fn=tf.nn.softmax, weights_initializer=w_init)
            out_vec.append(out)
        out = tf.concat([i for i in out_vec], axis=1)
...
#build CNN-based actor network
    def buildNetwork(self):
        _inputs = tf.placeholder(tf.float32, [None, self.__dim_s])
        net = _inputs
        # for more or less parameters
        #net = tf.contrib.layers.fully_connected(net, 128, activation_fn=tf.nn.leaky_relu)
        print("*******Build actor network(convolution)!*******")      
        net = self.myConv1d_first(net, 2, [2], 1)         
        net = self.myConv1d(net, 4, [2], 1)
        net = self.myConv1d_last(net, 1, [2], 1) 
...
#build ResNet-based critic network
    '''def buildNetwork(self):
        ...
        # for more or less parameters
        #net = tf.contrib.layers.fully_connected(net, 128, activation_fn=tf.nn.leaky_relu)
        print("*******Build actor network(Resnet)!*******")      
        net = self.myConv1d_first(net, 2, [2], 1)         
        net = self.myConv1d(net, 4, [2], 1)
        net = self.myConv1d_last(net, 1, [2], 1)
#add shotcut
...
#        net = tf.layers.conv1d(input_2d, out_dim, my_filter_size, stride, padding='SAME', activation=tf.nn.relu)          
    ...
    def buildNetwork(self):
        ...
        filter = tf.Variable(tf.random_normal([1, 2, 1, 4]))        
        net = self.myConv1d_first(net, filter, [1, 1, 1, 1])
        net = tf.nn.relu(net)                 
        #filter = tf.get_variable('weight-e', [1, 2, 4, 1], initializer=tf.truncated_normal_initializer(stddev=5e-2, dtype=tf.float32), dtype=tf.float32)          
